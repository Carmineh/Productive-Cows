...
public class AdvancedCoolerMenu extends AbstractContainerMenu {
    ...
    public AdvancedCoolerMenu(int windowId, Inventory inv, BlockEntity be, ContainerData data) {
        super(ProductiveCows.ADVANCED_COOLER_MENU.get(), windowId);
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            if (index < 6) {
                ...
                if (itemstack1.is(ProductiveCows.SPEED_UPGRADE.get())) {
                    if (!this.moveItemStackTo(itemstack1, 2, 3, false)) {
                        return ItemStack.EMPTY;
                    }
                } else if (itemstack1.is(ProductiveCows.EFFICIENCY_UPGRADE.get())) {
                    if (!this.moveItemStackTo(itemstack1, 3, 4, false)) {
                        return ItemStack.EMPTY;
                    }
                } else if (itemstack1.is(ProductiveCows.ENERGY_UPGRADE.get())) {
