...
public class BreedingRecipeCategory implements IRecipeCategory<CowDefinition> {
    ...
    public static final ResourceLocation UID = ResourceLocation.fromNamespaceAndPath(Materialcows.MODID, "breeding");
    public static final RecipeType<CowDefinition> BREEDING_TYPE = RecipeType.create(Materialcows.MODID, "breeding", CowDefinition.class);
    ...
    public BreedingRecipeCategory(IGuiHelper guiHelper) {
        ...
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(Materialcows.BREEDING_CAGE_ITEM.get()));
    }
    ...
    private ItemStack createEggItem(ResourceLocation defId) {
        if (defId == null) return ItemStack.EMPTY;
        ItemStack stack = new ItemStack(Materialcows.MATERIAL_COW_SPAWN_EGG.get());
        CompoundTag entityData = new CompoundTag();
        entityData.putString("id", "materialcows:material_cow");
        entityData.putString("CowDefinitionId", defId.toString());
        stack.set(DataComponents.ENTITY_DATA, CustomData.of(entityData));
