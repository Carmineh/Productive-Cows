...
public enum CowComponentProvider implements IEntityComponentProvider, IServerDataProvider<EntityAccessor> {
    ...
    public void appendTooltip(ITooltip tooltip, EntityAccessor accessor, IPluginConfig config) {
        if (accessor.getServerData().contains("MilkingCooldown")) {
            ...
            if (milking > 0) {
                int secs = milking / 20;
                tooltip.add(Component.translatable("tooltip.materialcows.jade.milking_cooldown", secs / 60, String.format("%02d", secs % 60)));
            } else {
                tooltip.add(Component.translatable("tooltip.materialcows.jade.milking_ready"));
            }
        }
        ...
        if (accessor.getServerData().contains("BreedingCooldown")) {
            ...
            if (breeding > 0) {
                ...
                tooltip.add(Component.translatable("tooltip.materialcows.jade.breeding_cooldown", secs / 60, String.format("%02d", secs % 60)));
            } else {
                tooltip.add(Component.translatable("tooltip.materialcows.jade.breeding_ready"));
            }
        }
    }
    ...
    public ResourceLocation getUid() {
        return ResourceLocation.fromNamespaceAndPath(Materialcows.MODID, "cow_cooldowns");
    }
}
