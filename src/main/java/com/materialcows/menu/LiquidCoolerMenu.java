...
public class LiquidCoolerMenu extends AbstractContainerMenu {
    ...
    public LiquidCoolerMenu(int windowId, Inventory inv, BlockEntity be, ContainerData data) {
        ...
        if (be instanceof LiquidCoolerBlockEntity cooler) {
            ...
            // Slot 2: Speed upgrade. X=152, Y=17
            this.addSlot(new SlotItemHandler(cooler.getInventory(), 2, 152, 17) {
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return stack.is(Materialcows.SPEED_UPGRADE.get());
                }
            });
            // Slot 3: Efficiency upgrade. X=152, Y=35
            this.addSlot(new SlotItemHandler(cooler.getInventory(), 3, 152, 35) {
                @Override
                public boolean mayPlace(ItemStack stack) {
